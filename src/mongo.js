module.exports =  class MongoWrapper {
  ...
  async next (id) {
    try{
      //checks if counter exists and returns value if it does, otherwise creates a new counter document
      const checkCounter = await this.dbo.collection("counters").findOneAndUpdate({_id: id},{$inc:{seq_value:1}},{returnNewDocument:true, upsert: true})
      if(checkCounter.value){
        return checkCounter.value.seq_value
      }else{
        //need to get intial value if counter did not exist
        const nextValue = await this.dbo.collection("counters").findOneAndUpdate({_id: id},{$inc:{seq_value:1}},{returnNewDocument:true})
        return nextValue.value.seq_value
      }
    }catch(e){
      throw (e)
    }
